	...
	if (my_turn == 0) {
		...
		if (previousFirst > 0) {
			if (100 - previousFirst < random_no + 1) {
				...
				document.getElementById((previousFirst).toString()).innerHTML = "";
			}
		}
		...
		if (previousSec > 0) {
			if (100 - previousSec < random_no + 1) {
				...
				document.getElementById((previousSec).toString()).innerHTML = "";
			}
		}
...
function snake(place_no, color_given, previous_player) {
	if (place_no == 26) {
		...
		document.getElementById('snakeMsg').innerHTML = "Sorry.. <p>You are Swallowed by the snake</p> <p> Better luck next time</p>";		
		document.getElementById('snakeMsg').style.color="#fff"
